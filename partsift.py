...
def find_monomials(polynomial, poly_degree, r, s):
	...
	test_monomial = 1	#This will be the base for all monomials we test with coefficient()
		...
		for power_set in multiset_permutations(power_values):
			...
			if max(power_set[0:r])<r and max(power_set[r:r+s])<s:
				...
				if polynomial_expanded.coefficient(temp_test_monomial) not in [0]:
					workable_monomials.append(polynomial_expanded.coefficient(temp_test_monomial)*temp_test_monomial)
...
def sift(r, s):
	...
	return find_monomials(poly_input[0], poly_input[1], r, s)
